/**
 * PR Review Wait Time API Endpoint
 * GET /api/metrics/review-wait
 *
 * Calculates median (p50) and 90th percentile (p90) PR review wait time for a given week.
 * Review wait time is measured from PR creation to first review, excluding draft PRs.
 *
 * NOTE: firstReviewAt is currently null for most PRs (enrichment planned in US2.1b).
 * Only PRs with review timestamps are included in calculations.
 *
 * Query Parameters:
 ...
 *
 * Examples:
 * - GET /api/metrics/review-wait (current week, all projects)
 * - GET /api/metrics/review-wait?week=2025-W02 (specific week, all projects)
 * - GET /api/metrics/review-wait?week=2025-W02&project=my-project (specific week and project)
 * - GET /api/metrics/review-wait?week=2025-W02&allProjects=true (per-project breakdown)
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (allProjectsParam === "true") {
      const projectMetrics = await calculatePRReviewWaitTimeByProject(
        startDate,
        endDate,
      );
      ...
      return NextResponse.json(
        {
          ...
          note: "Only PRs with review timestamps (firstReviewAt) are included. See schema.ts for enrichment details.",
        },
        { status: 200 },
      );
    }
    ...
    console.error("[API] Error in PR review wait time endpoint:", error);
