/**
 * Lead Time for Changes API Endpoint
 * GET /api/metrics/lead-time
 *
 * Calculates median (p50) and 90th percentile (p90) lead time for changes (PR creation to deployment).
 * This is one of the four DORA (DevOps Research and Assessment) metrics.
 *
 * NOTE: This metric requires deployments to have relatedPRs populated. If relatedPRs is empty,
 * the count will be 0 with null percentiles.
 *
 * Query Parameters:
 ...
 *
 * Examples:
 * - GET /api/metrics/lead-time (current week, all projects)
 * - GET /api/metrics/lead-time?week=2025-W02 (specific week, all projects)
 * - GET /api/metrics/lead-time?week=2025-W02&project=my-project (specific week and project)
 * - GET /api/metrics/lead-time?week=2025-W02&allProjects=true (per-project breakdown)
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (allProjectsParam === "true") {
      const projectMetrics = await calculateLeadTimeForChangesByProject(
        startDate,
        endDate,
      );
    ...
    const metrics = await calculateLeadTimeForChanges(
      startDate,
      endDate,
    ...
    console.error("[API] Error in lead time endpoint:", error);
